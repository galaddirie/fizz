...
const statusConfig = computed(() => {
  ...
  const adjust = (color: string, amount: number) =>
    isDark
      ? oklchToHex(darkenColor(color, amount))
      : oklchToHex(lightenColor(color, amount));
...
const currentStatusStyle = computed(
  () => statusConfig.value[effectiveStatus.value]
);
...
const nodeClasses = computed(() => [
  ...
  props.data.step_kind === "trigger" && !isSubnode.value
    ? "rounded-[50px_0.5rem_0.5rem_10px]"
    : "",
  props.dragging
...
const showStats = computed(() => {
  ...
  return (
    status &&
    status !== "pending" &&
    props.data.stats?.duration_us !== undefined
  );
});
...
const handleOutputQuickAdd = (screenPoint: { x: number; y: number }) => {
  ...
  sceneController?.step?.quickAdd?.(request);
};
...
const handleSubnodeSlotQuickAdd = (
  ...
  const request: StepHandleQuickAddRequest = {
    ...
    filter: {
      mode: "subnode_slot",
      accepted_type_ids:
        acceptedTypeIds.length > 0 ? acceptedTypeIds : undefined,
    },
  };
...
const commitName = () => {
  ...
  if (nextName !== (props.data.name || "Untitled Step")) {
    sceneController?.step?.update?.(props.id, { name: nextName });
  }
};
...
const handleTogglePin = () => {
  if (!canEdit.value) return;
  sceneController?.step?.togglePin?.(props.id, !!props.data.pinned);
};

const handleRunNode = () => {
  if (!canEdit.value || props.data.disabled) return;
  sceneController?.step?.run?.(props.id);
};

const handleToggleDisabled = () => {
  if (!canEdit.value) return;
  sceneController?.step?.toggleDisabled?.(props.id, !!props.data.disabled);
};
  ...
  <div class="relative inline-flex group">
    ...
    <PlayIcon
      ...
      :class="{
        ...
        'cursor-not-allowed hover:-translate-y-0 hover:text-base-content/70':
          data.disabled,
      }"
      aria-label="Run node"
      title="Run node"
    ...
    <template v-if="subnodeInputHandles.length > 0">
      ...
      >
        <span
          class="pointer-events-none mb-1 whitespace-nowrap text-[9px] font-medium text-base-content/50"
        >
          {{ slot.title || slot.id }}
        </span>
        <Handle
      ...
      <div class="flex w-full items-start gap-3">
        ...
        >
          <img
            ...
            alt=""
            :class="
              isSubnode ? 'size-5 object-contain' : 'size-6 object-contain'
            "
          />
          <component
            v-else
            ...
            :class="
              isSubnode
                ? 'text-base-content/75 size-5'
                : 'text-base-content/80 size-6'
            "
          />
        </div>
        ...
        <div class="min-w-0 flex-1">
          ...
          >
            ...
            <template v-if="data.itemStats?.isMultiItem">
              <span v-if="showStats" class="text-base-content/40 mx-0.5"
                >•</span
              >
              <span
                :class="[
                  ...
                  data.itemStats.completed === data.itemStats.itemsTotal
                    ? 'text-success'
                    : '',
                ]"
              >
