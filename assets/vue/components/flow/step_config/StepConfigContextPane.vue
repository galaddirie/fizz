...
<template>
  <div class="flex w-80 flex-col overflow-hidden border-r border-base-200/60">
    <template v-if="state.isManualTriggerStep.value && state.manualTriggerTestDataField.value">
      <div class="shrink-0 border-b border-base-200/60 px-4 py-4">
        <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-base-content/30">
          Test Data
        </p>
        <h3 class="mt-2 text-sm font-semibold text-base-content">
          Manual Trigger Payload
        </h3>
        <p class="mt-1 text-[11px] leading-relaxed text-base-content/45">
          Saved on this trigger and reused for both Run Test and Run from Here.
        </p>
      </div>

      <div class="custom-scrollbar flex-1 overflow-y-auto px-4 py-4">
        <JsonField
          :modelValue="state.fieldValues.value['test_data']"
          :field="state.manualTriggerTestDataField.value"
          :nodeId="state.nodeId.value"
          @update:modelValue="state.handleManualTriggerTestDataUpdate"
          @validation="error => state.handleFieldValidationUpdate('test_data', error)"
        />

        <div
          v-if="state.manualTriggerInputSchema.value"
          class="mt-5"
        >
          ...
          <div class="overflow-hidden rounded-xl border border-base-200/60 bg-base-100/40">
            <DataViewer
              :data="state.manualTriggerInputSchema.value"
              rootPath="input_schema"
