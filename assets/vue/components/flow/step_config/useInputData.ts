...
export function useInputData({
  ...
  const currentInputEmptyState = computed(() => {
    ...
    if (hasMultipleUpstream) {
      return {
        ...
        description:
          "Some upstream steps have data, but all inputs must be available before we can display them. Run to this step to compute the latest upstream outputs.",
      };
    }
  ...
  const runInputLabel = computed(() => {
    return directUpstreamStepIds.value.length === 1
      ? "Run previous step"
      : "Run to this step";
  });
