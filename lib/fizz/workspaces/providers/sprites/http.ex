defmodule Fizz.Workspaces.Providers.Sprites.Http do
  @moduledoc """
  ...
  @spec update_workspace_url_settings(String.t(), map()) :: response()
  def update_workspace_url_settings(remote_name, settings) when is_map(settings) do
    request(
      :put,
  ...
  def request(method, path, opts \\ []) do
    with token when is_binary(token) <- Client.api_key() do
      ...
      _ -> {:error, :workspace_provider_not_configured}
    end
  end
