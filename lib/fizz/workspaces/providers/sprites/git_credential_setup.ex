defmodule Fizz.Workspaces.Providers.Sprites.GitCredentialSetup do
  @moduledoc """
  ...
  def setup(remote_name, access_token, opts \\ []) do
    with {:ok, remote_workspace} <- Client.workspace(remote_name),
         :ok <- write_git_credentials(remote_workspace, access_token) do
  ...
  def teardown(remote_name) do
    with {:ok, remote_workspace} <- Client.workspace(remote_name) do
      delete_url =
        fs_url(remote_workspace, "/fs/delete", path: @git_credentials_path, recursive: "false")

      Req.delete(remote_workspace.client.req, url: delete_url)
      :ok
    else
      _ -> :ok
    end
  ...
  defp write_git_credentials(remote_workspace, access_token) do
    content = "https://x-access-token:#{access_token}@#{@github_host}\n"

    url =
      fs_url(remote_workspace, "/fs/write",
        path: @git_credentials_path,
        mode: "600",
        mkdirParents: "true"
      )

    case Req.put(remote_workspace.client.req, url: url, body: content) do
  ...
  defp fs_url(remote_workspace, endpoint, params) do
    name = URI.encode(remote_workspace.name)
