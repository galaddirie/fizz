defmodule Fizz.Workspaces.Workers.ExecJobWorker do
  @moduledoc """
  Executes queued workspace jobs and persists streamed logs.
  """
  ...
  alias Fizz.Repo
  alias Fizz.Workspaces, as: Broker
  alias Fizz.Workspaces.{ExecJob, ProviderRegistry}

  require Logger

  @impl Oban.Worker
  def perform(%Oban.Job{args: %{"exec_job_id" => exec_job_id}}) do
    case Repo.get(ExecJob, exec_job_id) |> Repo.preload([:workspace, :requested_by_user]) do
      nil ->
        :discard
  ...
  defp run(%ExecJob{} = exec_job) do
    ...
    with {:ok, provider} <- ProviderRegistry.provider(),
         {:ok, command} <-
           provider.start_exec(
             exec_job.workspace.remote_name,
             exec_job.command,
             exec_job.args,
             owner: self(),
      ...
      timeout_ms = exec_job.timeout_ms || provider.exec_timeout_ms_default()
      ...
      case collect_output(command, exec_job.id, start_ms, timeout_ms, 0, 0, 0, 0) do
        ...
        {:error, :closed, result} ->
          ...
          # This typically happens when the remote workspace closes TCP before the
  ...
  defp setup_git_credentials(%ExecJob{requested_by_user: %Accounts.User{} = user} = exec_job) do
    ...
    with {:ok, project_scope} <-
      ...
      case ProviderRegistry.provider() do
        {:ok, provider} ->
          case provider.setup_git_credentials(
                 exec_job.workspace.remote_name,
