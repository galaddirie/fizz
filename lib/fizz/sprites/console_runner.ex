defmodule Fizz.Sprites.ConsoleRunner do
  @moduledoc """
  ...
  def init(opts) do
    ...
    with {:ok, remote_sprite} <- Client.sprite(remote_name),
         {:ok, command} <-
           Sprites.spawn(remote_sprite, "bash", ["-i"],
             tty: true,
             stdin: true,
             tty_rows: rows,
             tty_cols: cols,
      ...
      {:error, reason} ->
        Logger.error("sprite console start failed: #{inspect(reason)}")
        {:stop, reason}
    end
  end
  ...
  def handle_info({:stdout, %{ref: ref}, data}, %{ref: ref} = state) do
    send(state.channel_pid, {:sprite_console_stdout, state.console_id, data})
    {:noreply, state}
  end

  def handle_info({:stderr, %{ref: ref}, data}, %{ref: ref} = state) do
    send(state.channel_pid, {:sprite_console_stderr, state.console_id, data})
    {:noreply, state}
  end

  def handle_info({:exit, %{ref: ref}, exit_code}, %{ref: ref} = state) do
    send(state.channel_pid, {:sprite_console_exit, state.console_id, exit_code})
    {:stop, :normal, state}
  end

  def handle_info({:error, %{ref: ref}, reason}, %{ref: ref} = state) do
    send(state.channel_pid, {:sprite_console_error, state.console_id, inspect(reason)})
    {:stop, :normal, state}
  end
