defmodule Fizz.Integrations.Steps.Registry do
  @moduledoc """
  ...
  ## Usage
      ...
      # Get all step types
      Fizz.Integrations.Steps.Registry.all()

      # Get a specific step type
      {:ok, type} = Fizz.Integrations.Steps.Registry.get("http_request")

      # List by category
      Fizz.Integrations.Steps.Registry.list_by_category("Integrations")

      # List by kind
      Fizz.Integrations.Steps.Registry.list_by_kind(:action)

  ## Adding New Step Types

  Create an executor module that uses `Fizz.Integrations.Steps.Definition`:

      defmodule Fizz.Integrations.Library.Fizz.Builtins.MyStep do
  ...
  defp builtin_executor_modules do
    Fizz.Integrations.Catalog.Manifest.step_executor_modules()
  end
  ...
  defp validate_credential_field_shape!(type, path, schema, default_config) do
    ...
    with true <- is_binary(requirement_key) and requirement_key != "",
         ...
         {:ok, _provider} <-
           Fizz.Integrations.Auth.ProviderCatalog.provider_for_type(provider, auth_atom),
