defmodule Fizz.Integrations.Catalog.IntegrationRegistry do
  @moduledoc """
  ...
  defp builtin_modules do
    Fizz.Integrations.Catalog.Manifest.integration_modules()
  end
  ...
  defp entry!(module) when is_atom(module) do
    with {:module, ^module} <- Code.ensure_loaded(module),
      ...
      _ ->
        raise ArgumentError,
              "#{inspect(module)} must implement Fizz.Integrations.Contracts.Integration"
    end
  end
  ...
  defp validate_provider!(%{id: id, provider_id: provider_id}) do
    case Fizz.Integrations.Auth.ProviderCatalog.provider(provider_id) do
      {:ok, _provider} ->
        :ok
  ...
  defp validate_step_definition!(module, entry) do
    if function_exported?(module, :__step_definition__, 0) do
      ...
      raise ArgumentError,
            "integration #{entry.id} step module #{inspect(module)} must use Fizz.Integrations.Steps.Definition"
    end
  end
