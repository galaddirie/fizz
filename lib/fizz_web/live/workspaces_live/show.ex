defmodule FizzWeb.WorkspacesLive.Show do
  ...
  @impl true
  def handle_params(%{"project_id" => project_id, "workspace_id" => workspace_id}, _uri, socket) do
    {:noreply,
     socket
     |> assign(:project_id, project_id)
     |> assign(:workspace_id, workspace_id)
  ...
  def handle_event("queue_job", %{"job" => params}, socket) do
    exec_spec = %{"command" => params["command"]}

    case Workspaces.queue_job(
  ...
  def handle_event("clone_repo", params, socket) when is_map(params) do
    ...
    with repo_id when is_binary(repo_id) <- repo_id,
      ...
      {:noreply,
       socket
       |> push_event("workspace_console_run_command", %{
  ...
  def handle_event("create_checkpoint", %{"checkpoint" => params}, socket) do
    case Workspaces.create_checkpoint(
           socket.assigns.current_scope,
           socket.assigns.project_id,
           socket.assigns.workspace_id,
  ...
  defp subscribe_to_job_topic(socket, job_id) do
    if connected?(socket) and not MapSet.member?(socket.assigns.subscribed_job_ids, job_id) do
      FizzWeb.Endpoint.subscribe("workspace_logs:#{job_id}")
  ...
  defp load_workspace(socket) do
    case Workspaces.get_workspace(
      ...
      {:error, :workspace_not_found} ->
        ...
        |> redirect(to: ~p"/projects/#{socket.assigns.project_id}/workspaces")

      {:error, reason} ->
        put_flash(socket, :error, "Could not load workspace: #{inspect(reason)}")
    end
  end

  defp load_jobs(socket) do
    case Workspaces.list_workspace_jobs(
           socket.assigns.current_scope,
           socket.assigns.project_id,
           socket.assigns.workspace_id
         ) do
  ...
  defp load_services(socket) do
    case Workspaces.list_workspace_services(
           socket.assigns.current_scope,
           socket.assigns.project_id,
           socket.assigns.workspace_id
         ) do
  ...
  defp load_checkpoints(socket) do
    case Workspaces.list_workspace_checkpoints(
           socket.assigns.current_scope,
           socket.assigns.project_id,
           socket.assigns.workspace_id
         ) do
