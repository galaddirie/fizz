defmodule FizzWeb.Api.SpriteController do
  ...
  def index(conn, %{"workspace_id" => workspace_id}) do
    case Sprites.list_workspace_sprites(conn.assigns.current_scope, workspace_id) do
  ...
  def create(conn, %{"workspace_id" => workspace_id} = params) do
    case Sprites.provision_sprite(conn.assigns.current_scope, workspace_id, params) do
      {:ok, sprite} ->
        conn
        |> put_status(:created)
  ...
  def show(conn, %{"workspace_id" => workspace_id, "sprite_id" => sprite_id}) do
    case Sprites.inspect_sprite(conn.assigns.current_scope, workspace_id, sprite_id) do
  ...
  def update(conn, %{"workspace_id" => workspace_id, "sprite_id" => sprite_id} = params) do
    case Sprites.reconfigure_sprite(conn.assigns.current_scope, workspace_id, sprite_id, params) do
      {:ok, sprite} -> json(conn, %{data: Helpers.sprite_json(sprite)})
      {:error, reason} -> Helpers.error(conn, reason)
    end
  end
  ...
  def delete(conn, %{"workspace_id" => workspace_id, "sprite_id" => sprite_id}) do
    case Sprites.terminate_sprite(conn.assigns.current_scope, workspace_id, sprite_id) do
      {:ok, sprite} -> json(conn, %{data: Helpers.sprite_json(sprite)})
      {:error, reason} -> Helpers.error(conn, reason)
    end
  end
