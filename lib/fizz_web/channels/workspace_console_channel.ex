defmodule FizzWeb.WorkspaceConsoleChannel do
  use FizzWeb, :channel

  alias Fizz.Integrations
  alias Fizz.Workspaces
  alias Fizz.Workspaces.ProviderRegistry

  require Logger

  @impl true
  def join("workspace_console:" <> console_id, _payload, socket) do
    scope = socket.assigns.current_scope

    with {:ok, console_session} <- Workspaces.authorize_console_session(scope, console_id),
         :active <- console_session.state,
         env_tuples = setup_git_credentials(scope, console_session),
         {:ok, provider} <- ProviderRegistry.provider(),
         {:ok, runner_pid} <-
           provider.start_console_runner(console_session, self(), env_tuples) do
  ...
  @impl true
  def handle_info(
        {:workspace_console_stdout, console_id, data},
  ...
  def handle_info(
        {:workspace_console_stderr, console_id, data},
        %{assigns: %{console_id: console_id}} = socket
      ) do
  ...
  def handle_info(
        {:workspace_console_exit, console_id, exit_code},
        %{assigns: %{console_id: console_id}} = socket
      ) do
  ...
  def handle_info(
        {:workspace_console_error, console_id, reason},
        %{assigns: %{console_id: console_id}} = socket
      ) do
    push(socket, "error", %{reason: reason})
    Workspaces.flag_console_error(console_id, reason)
  ...
  defp maybe_close_console(%{assigns: assigns} = socket, reason) do
    if assigns[:console_id] && assigns[:project_id] && assigns[:workspace_id] do
      _ =
  ...
  defp setup_git_credentials(scope, console_session) do
    ...
    with {:ok, token_result} <-
           Integrations.fetch_token_for_execution(scope, project_id, "github_oauth") do
      user_opts = git_user_opts(scope, project_id)

      case ProviderRegistry.provider() do
        {:ok, provider} ->
          case provider.setup_git_credentials(remote_name, token_result.access_token, user_opts) do
            {:ok, env_tuples} ->
              env_tuples
